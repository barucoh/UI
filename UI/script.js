...
$(document.getElementById("loginBtn")).click(function () {
    ...
    var password = document.getElementById("log_psw").value;
    $.ajax({
...
function setCookie(cvalue) {
    document.cookie = "usernamePicMe=" + cvalue + "";path=/";
}

function getCookie() {
    var name = "usernamePicMe=";
...
function checkCookie() {
    var user = getCookie("usernamePicMe");
    if (user !== "") {
        alert("Welcome again " + user);
    } else {
        document.getElementById('Login').style.display='none';
        user = document.getElementById("log_uname").value
        if (user !== "" && user != null) {
            setCookie(user);
        }
    }
